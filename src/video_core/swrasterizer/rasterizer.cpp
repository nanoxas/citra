...
std::tuple<Math::Vec4<u8>, Math::Vec4<u8>> ComputeFragmentsColors(const Math::Quaternion<float>& normquat, const Math::Vec3<float>& view) {
    ...
    auto normalized_normquat = Math::MakeVec<float>(normquat.xyz, normquat.w);
    normalized_normquat /= std::sqrt(normalized_normquat.x * normalized_normquat.x + normalized_normquat.y * normalized_normquat.y + normalized_normquat.z * normalized_normquat.z + normalized_normquat.w * normalized_normquat.w);

    auto normal = Math::QuaternionRotate({
        {normalized_normquat.x, normalized_normquat.y, normalized_normquat.z}, normalized_normquat.w
    }, surface_normal);
    ...
    for (unsigned light_index = 0; light_index <= lighting.max_light_index; ++light_index) {
        ...
        if (!lighting.IsDistAttenDisabled(num)) {
            ...
            u8 lutindex = MathUtil::Clamp(std::floorf(sample_loc * 256.f), 0.0f, 255.0f);
        ...
        auto GetLutIndex = [&](unsigned num, LightingRegs::LightingLutInput input,
            ...
            if (abs) {
                if (lighting.light[num].config.two_sided_diffuse)
                    result = fabs(result);
                else
                    result = std::max(result, 0.0f);

                u8 lutindex = MathUtil::Clamp(std::floorf(result * 256.f), 0.0f, 255.0f);
                float delta = result * 256 - lutindex;
                return { lutindex, delta };
            } else {
                float flr = std::floorf(result * 128.f);
